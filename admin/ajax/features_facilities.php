...
if (isset($_POST['add_facility'])) {
    ...
    $img_mime = $_FILES['icon']['type'];
    if (!in_array($img_mime, $valid_mime)) {
        return 'inv_img'; //invalid image mime or format
    } else if (($_FILES['icon']['size'] / (1024 * 1024)) > 1) {
        return 'inv_size'; //invalid size greater than 1mb
    } else {
        $ext = "svg";
        $rname = '\IMG_' . random_int(11111, 99999) . ".$ext";

        $img_path = $_SERVER['DOCUMENT_ROOT'] . '\facilities' . $rname;
        if (move_uploaded_file($_FILES['icon']['name'], $img_path)) {
            $img_r = $rname;
        } else {
            $img_r = 'upd_failed';
        }
